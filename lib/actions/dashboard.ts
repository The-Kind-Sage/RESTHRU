...
export async function getDashboardStats(restaurantId: string) {
  ...
  const [totalOrders, todayOrders, activeTables, pendingOrders] =
    await Promise.all([
      ...
      prisma.order.count({ where: { restaurantId, status: { in: ["PENDING", "PREPARING"] } } }),
    ]);

  const totalTables = await prisma.restaurantTable.count({ where: { restaurantId } });
...
export async function getRecentOrders(restaurantId: string, limit = 50) {
...
export async function getRevenueChartData(restaurantId: string, period: "week" | "month" = "week") {
  const now = new Date();
  ...
  for (const order of orders) {
    ...
    if (dailyMap.has(key)) dailyMap.set(key, (dailyMap.get(key) || 0) + order.totalAmount);
  }
  return Array.from(dailyMap.entries()).map(([date, revenue]) => ({ date, revenue })).sort((a, b) => a.date.localeCompare(b.date));
}
...
export async function getRecentActivity(restaurantId: string, limit = 5) {
  ...
  const activities: { id: string; type: "order" | "payment" | "cancelled"; title: string; time: Date }[] = [];
  for (const order of orders) {
    activities.push({
      id: `o-${order.id}`,
      type: order.status === "CANCELLED" ? "cancelled" : "order",
      title: order.status === "CANCELLED" ? `Order ${order.orderId} cancelled` : `Order ${order.orderId} placed`,
